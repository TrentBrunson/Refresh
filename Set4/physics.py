...
def chicken_up(startH, startV):
    ...
    for t in range(0, int(round(timeToMaxHeight + 1))):
        ...
        t += 1
    apex = hOFt + startH
    return apex, tableUp, t

def chicken_down(crest, finaltable, timeUp, ttf):
    """
    ...
    for time in [t/10 for t in range(0, int(round(ttf*10)))]:
        ...
        tableDown.append([round(time,1), heightDelta, descHeight])
        # only add whole seconds to table as to not have different units of output
        if time.is_integer(): 
            finaltable.append([time+timeUp, round(descHeight,2), round(crest + heightDelta,2)])
        if descHeight < 0:
            break
        time += 0.1
...
def outputs(table, height, velocity, apex, time, fallTime):
    print(f"{'Time':<10}{'Actual Height':<20}{'Total Distance':<20}")
    for i in range(len(table)):
        print(f"{round(table[i][0],1):<10}{table[i][1]:<20}{table[i][2]:<20}")
    print(f"Your starting parameters:\nHeight: {height} feet\nStarting velocity: {velocity} ft/sec"
    ...
    with open("object-trajectory.txt", "w") as f:
        ...
        for row in table:
            for field in row:
                f.writelines(f"{round(field,1):<15}")
            f.writelines("\n")        
...
def main():
    ht, vel = get_params()
    peak, upTable, upTime = chicken_up(ht, vel)
    tof, impactV = physics_check(peak)
    fTable, time = chicken_down(peak, upTable, tof, upTime)
